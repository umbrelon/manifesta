...
# SQL Server T-SQL with --include-migrations
# Use when PKs and FKs are declared via ALTER TABLE ADD CONSTRAINT after CREATE TABLE.
# Without this flag every table will have an empty primaryKey and no foreignKeys.
...
**SQL Server `--include-migrations`:**

Many SQL Server DDL scripts declare PKs and FKs in a separate `ALTER TABLE ADD CONSTRAINT` section that follows all `CREATE TABLE` blocks, rather than inline inside `CREATE TABLE`. Without `--include-migrations`, these are silently skipped and every table will have an empty `primaryKey` and zero `foreignKeys`. `validate cross` will report 0 errors — not because the schema is valid, but because there are no FKs to check.

Use `--include-migrations` whenever the T-SQL file was generated by SQL Server Management Studio or follows the `instawdb.sql` pattern.

**Views captured at parse time:**

`CREATE VIEW` statements are parsed and written as `table.json` files with `"isView": true`. Column names are extracted from the `SELECT` list; column types are set to `"unknown"` and resolved on the first `db merge` run. This means `validate cross`, `doc db`, and CI drift checks all see views immediately — without requiring a live database connection.
