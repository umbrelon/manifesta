...
- `db drift --ddl-file` — compare the schema registry against SQL DDL files without a live database connection; supports all four dialects including SQL Server T-SQL (the only `db drift` mode that accepts `--provider sqlserver` in OSS); accepts the same `--recursive`/`--pattern` glob inputs as `init sql`; parse errors block the comparison by default to prevent misleading reports (`--warn-only` overrides); `--ddl-file` is a third mutually-exclusive mode alongside the existing `--connection` and `--input-dir` modes
