...
### Added
- `tenants.databases[*].inputDir` and `tenants.databases[*].ddl` — per-entry offline sources in `manifesta.config.json` as alternatives to `connection`; each database in the topology independently uses live introspection (`connection`), pre-exported JSON (`inputDir`), or DDL file parsing (`ddl`); the three options are mutually exclusive per entry and validated by `TenantConfigValidator` with codes `TENANT-NO-SOURCE` and `TENANT-MULTIPLE-SOURCES`; SQL Server DDL accepted in the community edition (text-only parsing); enables air-gapped MVNO workflows and SQL Server tenant databases in OSS
- `db compare` — compare two databases directly and report schema differences without involving repository JSON files; each side (`--source`/`--source-ddl` and `--target`/`--target-ddl`) can independently be a live database connection or one or more DDL SQL files; `--source-ddl`/`--target-ddl` accept all four dialects including SQL Server T-SQL (pure text parsing, no live connection required); live connections follow the standard OSS provider rules (SQL Server live requires the full edition); `--strict` promotes warnings to errors; supports `--schema` for filtering or DDL prefix, `--include-schema` for detailed per-table field listings, and `--output`/`--output-dir` for report placement
...
- `db merge --ddl` — apply structural changes from SQL DDL files directly into the registry JSON files without a live database connection; supports all four dialects including SQL Server T-SQL; accepts comma-separated file paths; `--ddl` is a third mutually-exclusive mode alongside `--connection` and `--input-dir`; `--force-capture-reference-data` is incompatible with `--ddl`
